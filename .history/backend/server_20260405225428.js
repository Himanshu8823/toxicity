...
app.post('/analyze-video', async (req, res) => {
  try {
    ...
    for (const r of valid) {
      ...
      labelScores[normalized] = (labelScores[normalized] || 0) + r.confidence;
    }
    ...
    // Properly categorize toxic vs non-toxic
    const toxicResults = valid.filter((r) => {
      const normalized = normalizeLabel(r.mostLikelyCategory);
      return HARMFUL_LABELS.has(normalized);
    });
    
    const nonToxicResults = valid.filter((r) => {
      const normalized = normalizeLabel(r.mostLikelyCategory);
      return normalized === 'non_toxic';
    });
