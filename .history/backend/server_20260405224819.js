...
app.post('/analyze-video', async (req, res) => {
  try {
    ...
    const toxicResults = valid.filter((r) => HARMFUL_LABELS.has(normalizeLabel(r.mostLikelyCategory)));
    const nonToxicResults = valid.filter((r) => normalizeLabel(r.mostLikelyCategory) === 'non_toxic');

    // Top 10 most toxic comments by confidence
    const mostToxic = valid
      .filter((r) => HARMFUL_LABELS.has(normalizeLabel(r.mostLikelyCategory)))
